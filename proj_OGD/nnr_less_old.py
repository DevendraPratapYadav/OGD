...
def apply_regression(filename, inCol, outCol, predValues):
	...
	degree,rank = load_all(filename, inCol, outCol);
	...
	for rr in xrange(0,runs):
		...
		"""
		svr = SVR(C=30, cache_size=200, epsilon=0.00001, gamma=50,kernel='rbf', max_iter=-1, shrinking=True, tol=0.000001, verbose=False)
		...
		# calculate deviation from true rank for each test instance
...
# Pr = apply_regression('inp.txt',[0],[1],[[0,2013,8,1]]);
# printV (Pr);